...
Kimi Code 在 `$KIMI_CODE_HOME/sessions/`（默认 `~/.kimi-code`）下用 `state.json` 保存会话元数据，并在 `agents/<agent>/wire.jsonl` 中保存完整的有序事件流。Session Exporter 会读取主 Agent 和所有子 Agent 的日志，按时间合并转录事件，并且只汇总规范的 `usage.record`，避免重复计算同一次生成用量。

标准化字段 `inputOther`、`output`、`inputCacheRead` 与 `inputCacheCreation` 提供准确的 token 与缓存统计。应用会尊重已配置的 `KIMI_CODE_HOME`。已知的宿主指令包装会从列表标题和转录中移除，普通 Kimi CLI 提示词保持原样。
