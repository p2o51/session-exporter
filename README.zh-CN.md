...
**浏览并导出你的 Claude Code / Codex / Cursor 会话历史 —— 附带 token 与花费统计。**
...
一个小巧、典雅的**本地**网页工具：读取你的 **Claude Code**、**Codex**、**Cursor** 会话历史，
让你浏览、筛选并导出，附带真实的 token 统计（含缓存命中）和**缓存感知的花费估算**。
所有数据都不出本机。

## 功能

- **三个工具，一个列表** —— 所有 Claude Code / Codex / Cursor 会话汇总在一起。可按来源、
  项目文件夹、日期区间、全文搜索筛选；可按时间、花费、token、大小排序。
...
- **本地且私密** —— 纯 Python 3.9+ 标准库，**零依赖**。Cursor 数据库严格只读打开。
...
parsers/          claude.py · codex.py · cursor.py（每个来源一份契约）
web/              index.html · styles.css · app.js（界面）
website/          Rspress 文档站（三语）
```

每个解析器实现一份很小的契约（`list_sessions()` / `load_messages()`），所以新增第四个来源
