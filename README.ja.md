...
**Claude Code / Codex / Cursor / Antigravity / Pi Agent / Kimi Code の履歴を閲覧・エクスポート — トークンとコストの集計付き。**
...
**Antigravity**・**Pi Agent**・**Kimi Code** のセッション履歴を読み込み、閲覧・フィルタ・エクスポートできます。実測の
トークン集計（キャッシュヒットを含む）と、キャッシュを考慮した**コスト見積もり**付き。
データは一切外部に出ません。

## 特長

- **6 つのツールを 1 つのリストに** — Claude Code / Codex / Cursor / Antigravity / Pi Agent / Kimi Code の全セッションを
  まとめて表示。ソース・プロジェクトフォルダ・期間・全文検索でフィルタでき、更新日・コスト・
  トークン・サイズで並べ替えられます。
- **複数選択 → エクスポートメニュー** — 選択または全選択（現在のフィルタに追従）して、ZIP アーカイブ、
  Notion インポートパック、JSON、Markdown レポートとして出力できます（画像エクスポートは近日対応）。
- **キャッシュ込みのトークン集計** — プロバイダが実際に記録した 入力 / 出力 / キャッシュ読取 /
  キャッシュ書込 / 推論 トークンと、キャッシュヒット率を、セッションごと・選択ごとに集計します。
- **コスト見積もり** — 各セッションを トークン × モデル別単価 で計算。プロバイダー別のキャッシュ読取と
  Claude / Pi / Kimi のキャッシュ書込を正しく計上します。選択バーを展開すると**モデル別**・**日付別**の
  **Stats** が表示されます。単価は編集可能な [`pricing.json`](pricing.json) にあります。
- **ローカルかつプライベート** — 純粋な Python 3.9+ 標準ライブラリのみ、**依存関係ゼロ**。
  Cursor や Antigravity などのローカルデータベースは厳密に読み取り専用で開きます。
...
parsers/          claude · codex · cursor · antigravity · pi · kimi（ソースごとに 1 契約）
